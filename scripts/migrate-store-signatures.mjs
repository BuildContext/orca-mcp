...
function printUsage(stream = console.error) {
  stream(
    [
      ...
      '',
      'Sign legacy unsigned ownership stores so 0.3.3 load accepts them.',
      'Runtime still rejects unsigned — this is an explicit operator step.',
      '',
      'Production invocation (join the signer group; sudo -u orca always EACCES):',
      '  setpriv --reuid=orca --regid=orca --groups orca-bridge-signer \\',
      '    env HOME=/home/orca ORCA_BRIDGE_STORE_SIGNER_SOCKET=/run/orca-bridge/store-signer.sock \\',
      '    node scripts/migrate-store-signatures.mjs',
      '',
      'Options:',
