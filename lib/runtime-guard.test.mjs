  ...
  it('throws runtime_unavailable when status probe fails', () => {
    assert.throws(
      ...
      (err) => {
        ...
        assert.match(err.recovery, /ORCA_CLI_COMMAND|status --json/i);
        return true;
      },
    );
  });
...
describe('compactHealthPayload', () => {
  ...
  it('keeps client-critical fields and drops giant dumps', () => {
    const c = compactHealthPayload(full);
    assert.equal(c.ok, true);
    assert.equal(c.bridge.version, '0.3.0');
    assert.equal(c.bridge.versionOk, true);
    assert.equal(c.statusProbe.ok, true);
    assert.equal(c.statusProbe.exitCode, 0);
    assert.equal(c.senderTerminal.handle, 'term_abc');
    assert.equal(c.verbose, false);
    assert.equal(c.next.action, 'dispatch_or_guide');
    assert.ok(c.toolsets);
    assert.deepEqual(c.toolsets.enabled, ['orchestration', 'worktree']);
    ...
    assert.equal(c.bridge.node, undefined);
  });
  ...
  it('reports ok=false when versionOk false', () => {
    ...
    assert.equal(c.bridge.versionOk, false);
  });
