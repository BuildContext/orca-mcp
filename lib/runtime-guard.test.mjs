...
describe('doctrine NAS-246/240', () => {
  ...
  it('raw_cli_ok is the exact-form contract (NAS-254), not prefix prose', () => {
    const g = buildCoordinatorGuide({ version: '0.3.5', minVersion: '0.2.0' });
    const blob = g.raw_cli_ok.join('\n');
    assert.match(blob, /exact command (path|form)/i);
    assert.match(blob, /--peek/);
    assert.match(blob, /--all/);
    assert.match(blob, /OK-tier/i);
    assert.match(blob, /ADMIN-tier/i);
    assert.match(blob, /orchestration check/);
    assert.match(blob, /terminal send/);
    assert.equal(/orchestration reply \(questions\)/.test(blob), false);
  });
});
