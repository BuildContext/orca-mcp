...
export const STORE_SIG_VERSION = 1;
 ...
 * @param {string} sig
 * @returns {{ v: number, alg: string, payload: unknown, sig: string }}
 */
export function makeSignedEnvelope(payload, sig) {
  return {
    v: STORE_SIG_VERSION,
    alg: STORE_SIG_ALG,
    payload,
...
/**
 * True when value looks like our signed envelope (shape only, no crypto).
 * @param {unknown} value
 */
 ...
 * @param {Buffer|string} key
 */
export function createLocalHmacSigner(key) {
  ...
  return {
    ...
    async sign(payload) {
      ...
      return { ok: true, sig: hmacSign(keyBuf, canonical), canonical };
    },
    async verify(payload, sig) {
      const canonical = canonicalJson(payload);
      const valid = hmacVerify(keyBuf, canonical, sig);
      return { ok: true, valid, canonical };
    },
  };
}
...
export function createSocketSigner(socketPath, opts = {}) {
  ...
  return {
    ...
    socketPath,
    async sign(payload) {
      const msg = await request({ op: 'sign', payload });
      ...
      return { ok: true, sig: msg.sig };
    },
    async verify(payload, sig) {
      const msg = await request({ op: 'verify', payload, sig });
      if (!msg || msg.ok !== true) {
        return {
          ok: false,
          valid: false,
 ...
 * @param {unknown} payload
 * @param {{ sign: (p: unknown) => Promise<{ok:boolean,sig?:string,error?:string}> }} signer
 */
export async function signPayload(payload, signer) {
  if (!signer || typeof signer.sign !== 'function') {
    return { ok: false, reason: SIGNED_STORE_SIGNER_UNAVAILABLE };
  }
  let result;
  try {
    result = await signer.sign(payload);
  } catch (e) {
    return {
      ok: false,
  ...
  return { ok: true, envelope: makeSignedEnvelope(payload, result.sig) };
}

/**
 * Verify a signed envelope. Rejects unsigned / malformed / bad sig.
 * @param {unknown} envelope
 * @param {{ verify: (p: unknown, sig: string) => Promise<{ok:boolean,valid?:boolean,error?:string}> }} signer
 */
export async function verifyEnvelope(envelope, signer) {
  if (!signer || typeof signer.verify !== 'function') {
    return { ok: false, reason: SIGNED_STORE_SIGNER_UNAVAILABLE };
  }
  if (!isSignedEnvelope(envelope)) {
    // Distinguish completely foreign JSON (legacy unsigned store) from broken envelope.
...
export async function readSignedJsonFile(filePath, signer, deps = {}) {
  ...
  return { ok: true, payload: verified.payload, path: filePath };
}
 ...
 * @param {Buffer} key
 */
export function handleSignerRequest(msg, key) {
  ...
  if (op === 'sign') {
    ...
    const sig = hmacSign(key, canonicalJson(msg.payload));
    return { id: msg.id, ok: true, sig, alg: STORE_SIG_ALG };
  }
  if (op === 'verify') {
  ...
  server.on('connection', (socket) => {
    ...
    socket.on('data', (chunk) => {
      ...
      while ((nl = buf.indexOf('\n')) !== -1) {
        ...
        let response;
        try {
          response = handleSignerRequest(msg, key);
        } catch (e) {
          response = {
            id: msg && msg.id,
            ok: false,
