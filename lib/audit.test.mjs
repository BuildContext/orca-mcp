...
describe('MCP resources list/read', () => {
  ...
  beforeEach(() => {
    ...
    const bound = registry.bindOwner('disp_1', {
      status: 'running',
      runId: 'run_1',
      clientKey: 'client-a',
...
// ---------------------------------------------------------------------------
// NAS-248 P0 #4 — ownership binding is authoritative, not claimable
// ---------------------------------------------------------------------------

describe('dispatchRegistry ownership bind (NAS-248 P0 #4)', () => {
  it('upsert never sets or overwrites clientKey', () => {
    const registry = createDispatchRegistry();
    // Claim attempt via plain upsert — must not stick.
    registry.upsert('disp_bob', {
      status: 'worker_done',
      clientKey: 'alice',
      runId: 'run_x',
    });
    assert.equal(registry.get('disp_bob')?.clientKey, undefined);

    const bound = registry.bindOwner('disp_bob', {
      clientKey: 'bob',
      terminalHandle: 'term_bob',
      status: 'running',
    });
    assert.equal(bound.ok, true);
    assert.equal(registry.get('disp_bob').clientKey, 'bob');

    // Attacker await-shaped upsert must not reassign.
    registry.upsert('disp_bob', {
      status: 'worker_done',
      clientKey: 'alice',
      terminalHandle: 'term_alice_forged',
    });
    const row = registry.get('disp_bob');
    assert.equal(row.clientKey, 'bob');
    assert.equal(row.terminalHandle, 'term_bob');
    assert.equal(row.status, 'worker_done');
  });

  it('bindOwner refuses reassignment to a different client', () => {
    const registry = createDispatchRegistry();
    assert.equal(
      registry.bindOwner('disp_1', { clientKey: 'bob', terminalHandle: 'term_b' }).ok,
      true,
    );
    const steal = registry.bindOwner('disp_1', {
      clientKey: 'alice',
      terminalHandle: 'term_a',
    });
    assert.equal(steal.ok, false);
    assert.equal(steal.reason, 'owner_mismatch');
    assert.equal(registry.get('disp_1').clientKey, 'bob');
  });

  it('bindOwner is idempotent for the same owner', () => {
    const registry = createDispatchRegistry();
    assert.equal(
      registry.bindOwner('disp_1', { clientKey: 'alice', status: 'running' }).ok,
      true,
    );
    const again = registry.bindOwner('disp_1', {
      clientKey: 'alice',
      status: 'worker_done',
      outcome: 'succeeded',
    });
    assert.equal(again.ok, true);
    assert.equal(registry.get('disp_1').clientKey, 'alice');
    assert.equal(registry.get('disp_1').status, 'worker_done');
  });

  it('listOwnershipBindings only exports bound rows', () => {
    const registry = createDispatchRegistry();
    registry.upsert('orphan', { status: 'seen' });
    registry.bindOwner('disp_a', {
      clientKey: 'alice',
      terminalHandle: 'term_a',
      runId: 'run_1',
    });
    const bindings = registry.listOwnershipBindings();
    assert.equal(bindings.length, 1);
    assert.equal(bindings[0].dispatchId, 'disp_a');
    assert.equal(bindings[0].clientKey, 'alice');
    assert.equal(bindings[0].terminalHandle, 'term_a');
    assert.equal(bindings[0].workerIsolation, undefined);
    assert.equal(bindings[0].workerCapabilityMinted, undefined);
  });

  it('listOwnershipBindings preserves explicit isolation/mint flags', () => {
    const registry = createDispatchRegistry();
    registry.bindOwner('disp_iso', {
      clientKey: 'alice',
      terminalHandle: 'term_a',
      workerIsolation: true,
      workerCapabilityMinted: true,
    });
    registry.bindOwner('disp_off', {
      clientKey: 'alice',
      terminalHandle: 'term_b',
      workerIsolation: false,
      workerCapabilityMinted: false,
    });
    const byId = Object.fromEntries(registry.listOwnershipBindings().map((b) => [b.dispatchId, b]));
    assert.equal(byId.disp_iso.workerIsolation, true);
    assert.equal(byId.disp_iso.workerCapabilityMinted, true);
    assert.equal(byId.disp_off.workerIsolation, false);
    assert.equal(byId.disp_off.workerCapabilityMinted, false);
  });
});
