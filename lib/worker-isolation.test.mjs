...
describe('planIsolatedAgentPlacement', () => {
  ...
  it('buildWorkerLaunchCommand binds --task-id when safe', () => {
    assert.equal(
      buildWorkerLaunchCommand('grok', cfgOn, { taskId: 'task_582ffae59441' }),
      '/usr/local/lib/orca-mcp/orca-omp-as-worker.sh --task-id task_582ffae59441 grok',
    );
    assert.equal(
      buildWorkerLaunchCommand('grok', cfgOn, { taskId: '../etc/passwd' }),
      '/usr/local/lib/orca-mcp/orca-omp-as-worker.sh grok',
    );
  });
});
...
describe('workerIsolationHealth', () => {
  it('states per-dispatch uid is NOT shipped', () => {
    ...
    assert.equal(h.perWorktreeAcl, true);
    assert.match(h.note, /NOT shipped/);
    assert.match(h.note, /per-worktree ACL/);
