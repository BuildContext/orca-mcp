...
export async function migrateOneStoreFile(filePath, signer, opts = {}) {
  ...
  // Path A: already a v2 signed envelope — verify, never re-sign on bad sig.
  ...
  // Path A2: v1 envelope with a valid payload-only MAC → re-sign as v2.
  // Invalid v1 is refuse (do not launder). Runtime load rejects v1.
  if (isV1SignedEnvelope(parsed)) {
    if (!signer || typeof signer.verifyLegacyV1 !== 'function') {
      return {
        ok: false,
        path: filePath,
        label,
        action: 'signer-unavailable',
        reason: SIGNED_STORE_SIGNER_UNAVAILABLE,
        error: 'signer cannot verify v1 envelopes',
      };
    }
    let v1;
    try {
      v1 = await signer.verifyLegacyV1(parsed.payload, parsed.sig);
    } catch (e) {
      return {
        ok: false,
        path: filePath,
    ...
    if (!v1 || v1.ok !== true) {
      return {
        ...
        action: 'signer-unavailable',
        reason: SIGNED_STORE_SIGNER_UNAVAILABLE,
        error: (v1 && v1.error) || 'verify-v1 failed',
      };
    }
    if (v1.valid !== true) {
      return {
        ...
        label,
        action: 'refused-bad-signature',
        reason: SIGNED_STORE_BAD_SIG,
        error: 'v1 envelope MAC invalid',
      };
    }
    if (dryRun) {
      return {
    ...
    const written = await writeSignedJsonFile(filePath, parsed.payload, signer, {
