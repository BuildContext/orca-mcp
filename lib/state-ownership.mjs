...
// argv extraction outside collectTerminalHandlesFromArgv / collectDispatchIdsFromArgv.
// ---------------------------------------------------------------------------
...
/**
 * Redact foreign scrollback from a `terminal list` JSON payload.
 *
 * Decision (NAS-248): omit the `preview` field for terminals the caller does
 * not own; keep the row (handle/title/path/state). Coordinators need inventory
 * of other terminals; they must not read foreign PTY content (NAS-218/170 class).
 *
 * Apply only at the action=cli response boundary — never inside runJson used by
 * resolveSenderTerminal (title rediscovery needs full inventory, not preview).
 *
 * @param {unknown} payload envelope.result or raw list object
 * @param {Iterable<string>|Set<string>} ownedHandles
 * @returns {unknown} shallow-copied structure with previews stripped
 */
export function redactTerminalListPayload(payload, ownedHandles) {
  ...
  const redactTerminal = (term) => {
    if (!term || typeof term !== 'object') return term;
    const handle = normalizeTerminalHandle(term.handle);
    if (handle && owned.has(handle)) return term;
    if (!Object.prototype.hasOwnProperty.call(term, 'preview')) return term;
    const copy = { ...term };
    delete copy.preview;
    return copy;
  };

  // Common shapes: { terminals: [...] }, { result: { terminals } }, bare array
  if (Array.isArray(payload)) {
    return payload.map(redactTerminal);
  }

  const out = { ...payload };
  if (Array.isArray(out.terminals)) {
    out.terminals = out.terminals.map(redactTerminal);
  }
  if (out.result && typeof out.result === 'object' && !Array.isArray(out.result)) {
    const result = { ...out.result };
    if (Array.isArray(result.terminals)) {
      result.terminals = result.terminals.map(redactTerminal);
    }
    out.result = result;
  }
  return out;
...
/**
 * Redact worktree list `preview` fields (sibling of terminal list leak).
 * Omits preview on each worktree row; keeps the row.
 *
...
export function redactWorktreeListPayload(payload) {
  ...
  const redactWt = (wt) => {
    if (!wt || typeof wt !== 'object') return wt;
    if (!Object.prototype.hasOwnProperty.call(wt, 'preview')) return wt;
    const copy = { ...wt };
    delete copy.preview;
    return copy;
  };

  if (Array.isArray(payload)) return payload.map(redactWt);

  const out = { ...payload };
  if (Array.isArray(out.worktrees)) {
    out.worktrees = out.worktrees.map(redactWt);
  }
  if (out.result && typeof out.result === 'object' && !Array.isArray(out.result)) {
    const result = { ...out.result };
    if (Array.isArray(result.worktrees)) {
      result.worktrees = result.worktrees.map(redactWt);
    }
    out.result = result;
  }
  return out;
...
/**
 * True when a value looks like a terminal-list payload (rows with handle + optional preview).
 * @param {unknown} value
 * @returns {boolean}
 */
function looksLikeTerminalListPayload(value) {
  ...
  if (Array.isArray(value)) {
    return value.some(
      (row) =>
        row &&
        typeof row === 'object' &&
        (Object.prototype.hasOwnProperty.call(row, 'handle') ||
          Object.prototype.hasOwnProperty.call(row, 'preview') ||
          Object.prototype.hasOwnProperty.call(row, 'scrollback')),
    );
  }
  if (Array.isArray(value.terminals)) return true;
  if (
    value.result &&
    typeof value.result === 'object' &&
    Array.isArray(value.result.terminals)
  ) {
    return true;
  }
  return false;
...
/**
 * True when a value looks like a worktree-list payload with preview rows.
 * @param {unknown} value
 * @returns {boolean}
 */
function looksLikeWorktreeListPayload(value) {
  if (value == null || typeof value !== 'object') return false;
  if (Array.isArray(value)) {
    return value.some(
      (row) =>
        ...
        typeof row === 'object' &&
        Object.prototype.hasOwnProperty.call(row, 'preview') &&
        (Object.prototype.hasOwnProperty.call(row, 'path') ||
          Object.prototype.hasOwnProperty.call(row, 'id') ||
          Object.prototype.hasOwnProperty.call(row, 'worktreeId')),
    );
  }
  if (Array.isArray(value.worktrees)) return true;
  if (
    value.result &&
    typeof value.result === 'object' &&
    Array.isArray(value.result.worktrees)
  ) {
    return true;
  }
  return false;
...
/**
 * Redact foreign `preview:` lines from human `terminal list` text.
...
export function redactTerminalListHumanStdout(text, ownedHandles) {
  if (typeof text !== 'string' || !text.includes('preview:')) return text;
  const owned = new Set();
  for (const h of ownedHandles || []) {
    const n = normalizeTerminalHandle(h);
    if (n) owned.add(n);
  ...
  for (const line of lines) {
    ...
    if (trimmed.startsWith('preview:')) {
      if (currentHandle && owned.has(currentHandle)) {
        ...
        // foreign / unknown — drop scrollback content, keep a placeholder so
        // the row structure stays parseable for humans.
        const indent = line.slice(0, line.length - trimmed.length);
        out.push(`${indent}preview: <redacted>`);
      } else {
        // No preceding handle line — fail closed.
        const indent = line.slice(0, line.length - trimmed.length);
        out.push(`${indent}preview: <redacted>`);
      }
      continue;
...
/**
 * Shape-based ownership redaction for action=cli responses (NAS-248 F2).
 *
 * Does NOT inspect argv. If the response carries terminal rows with
 * preview/scrollback, non-owned rows lose that field — envelope or stdout,
 * JSON or human. Same for worktree preview rows.
 *
...
export function applyOwnershipListRedaction(described, ownedHandles) {
  ...
  const redactPayload = (payload) => {
    if (looksLikeTerminalListPayload(payload)) {
      return redactTerminalListPayload(payload, ownedHandles);
    }
    if (looksLikeWorktreeListPayload(payload)) {
      return redactWorktreeListPayload(payload);
    }
    return payload;
  };
  ...
  if (described.envelope && typeof described.envelope === 'object') {
    ...
    if (env.result != null && typeof env.result === 'object') {
      const nextResult = redactPayload(env.result);
      if (nextResult !== env.result) {
        described.envelope = { ...env, result: nextResult };
      }
    } else {
      // envelope itself may be the list payload
      const nextEnv = redactPayload(env);
      if (nextEnv !== env) described.envelope = nextEnv;
    }
  }
  ...
  if (typeof described.stdout === 'string' && described.stdout) {
    ...
    if (trimmed.startsWith('{') || trimmed.startsWith('[')) {
      try {
        ...
        const next = redactPayload(parsed);
        if (next !== parsed) {
          described.stdout = JSON.stringify(next);
        }
      } catch {
        // not JSON — fall through to human redaction
        described.stdout = redactTerminalListHumanStdout(raw, ownedHandles);
      }
    } else if (raw.includes('preview:')) {
      described.stdout = redactTerminalListHumanStdout(raw, ownedHandles);
    }
  }
