...
/**
 * Normalize compound --to values (run:ID / dispatch:ID) to the bare id.
 * @param {unknown} raw
 */
...
export function redactTerminalListHumanStdout(text, ownedHandles) {
  ...
  // No keyword gate: a redactor that only runs when it recognises a word is not a redactor.
  const owned = buildOwnedHandleSet(ownedHandles);
  const lines = text.split('\n');
  /** @type {string|null} */
  let currentHandle = null;
  const out = [];
  const contentLine = /^(preview|scrollback|buffer|output|lines|tail|snapshot|subject|body|payload|prompt|spec|objective|preamble)\s*:/i;
  const jsonKey = /^"?(preview|scrollback|buffer|output|lines|tail|snapshot|subject|body|payload|prompt|spec|objective|preamble|title|worktreePath|worktree_path|path)"?\s*:/i;
  for (const line of lines) {
    const trimmed = line.trimStart();
    const jk = trimmed.match(jsonKey);
    const ck = trimmed.match(contentLine);
    if (jk || ck || trimmed.toLowerCase().startsWith('preview:')) {
      if (currentHandle && owned.has(currentHandle)) {
        out.push(line);
      } else {
        const indent = line.slice(0, line.length - trimmed.length);
        const key = (jk && jk[1]) || (ck && ck[1]) || 'preview';
        if (trimmed.startsWith('"') || (jk && trimmed.startsWith('"'))) {
    ...
    // Human terminal list rows: handle  title  state  path — strip title/path when unowned
    const row = trimmed.match(/^(term_[A-Za-z0-9_-]+)\b(.*)$/);
    if (row) {
      currentHandle = normalizeTerminalHandle(row[1]);
      if (currentHandle && !owned.has(currentHandle)) {
        // Keep handle + coarse state tokens; drop free-text title/paths
        const rest = row[2] || '';
        const stateBits = rest.match(/\b(connected|disconnected|writable|orphaned|running|idle|exited)\b/gi) || [];
        out.push(`${row[1]}${stateBits.length ? '  ' + stateBits.join(' ') : ''}`);
        continue;
      }
      out.push(line);
      continue;
    }
    // Inbox human lines: msg_… from -> to: "subject"
    const msgRow = trimmed.match(/^(msg_[A-Za-z0-9_-]+)\b/);
    if (msgRow) {
      // Without a proven owned handle on the line, strip subject/body/payload markers
      if (/\[payload\]/i.test(line) || /:s*"/.test(line)) {
        out.push(`${msgRow[1]} <redacted>`);
        continue;
      }
    }
    ...
    // Drop lines that are pure payload/body dumps without inventory ids
    if (/^\[payload\]/i.test(trimmed) || /^body:/i.test(trimmed) || /^subject:/i.test(trimmed)) {
      if (!(currentHandle && owned.has(currentHandle))) {
        out.push('<redacted>');
        continue;
      }
...
function redactTextField(raw, ownedHandles) {
  ...
  if (parsed != null) {
    return JSON.stringify(redactOwnershipContent(parsed, ownedHandles));
  }
  return redactTerminalListHumanStdout(raw, ownedHandles);
}
...
export function applyOwnershipListRedaction(described, ownedHandles) {
  ...
  if (typeof described.stdout === 'string' && described.stdout) {
    described.stdout = redactTextField(described.stdout, ownedHandles);
  }
  if (typeof described.stderr === 'string' && described.stderr) {
    described.stderr = redactTextField(described.stderr, ownedHandles);
  }
  if (typeof described.stderrTail === 'string' && described.stderrTail) {
    described.stderrTail = redactTextField(described.stderrTail, ownedHandles);
  }
