 ...
 *
 * When the process IS root, two situations orphan the service account and both
 * are handled here:
 *   - the file exists (possibly just replaced by a fresh inode from a migration
 *     `os.replace()`): restore the uid/gid it had before this write;
 *   - the file does not exist yet: fall back to the owner of the containing
 *     directory, which for `~/.orca-bridge-*` is the service account's home.
 *
...
export function writeFilePreservingOwner(filePath, data, opts = {}) {
  ...
  try {
    ...
    previousOwner = { uid: st.uid, gid: st.gid, from: 'file' };
  } catch {
    // First write. The state lives in the service account's HOME, so the
    // directory owner is the account this file should belong to.
    try {
      const dst = fsImpl.statSync(path.dirname(filePath));
      previousOwner = { uid: dst.uid, gid: dst.gid, from: 'directory' };
    } catch {
      previousOwner = null;
    }
  }
