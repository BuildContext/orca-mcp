 ...
 * @returns {string|null}
 */
function collapsePathDots(rawPath) {
  // POSIX-ish collapse of '.' / '..' and trailing slashes for ownership compare.
  // Does not resolve symlinks; only token-level normalisation so path:/a/ and
  // path:/a/. and path:/a/b/../a compare as the same selector family.
...
export function normalizeWorktreeSelector(sel) {
  ...
  if (lower.startsWith('path:')) {
    const p = collapsePathDots(s.slice(5));
  ...
  if (lower.startsWith('branch:')) {
    const b = s.slice(7);
    return b ? `branch:${b}` : null;
  }
  if (lower.startsWith('issue:')) {
    const iss = s.slice(6);
    return iss ? `issue:${iss}` : null;
  }
  if (s.startsWith('/')) {
    const p = collapsePathDots(s);
    return p ? `path:${p}` : null;
  }
...
/**
 * Value-typed ownership kind from the REFERENT grammar, not the flag name.
 * Returns a resolver kind or null when the value is not an ownership referent.
 * @param {unknown} raw
 * @returns {'handle'|'dispatch'|'worktree'|'task'|'run'|'id'|null}
 */
export function classifyValueOwnershipKind(raw) {
  if (raw == null) return null;
  const v = String(raw).trim();
  if (!v) return null;
  // @worktree:<sel> is a documented --to group address; strip handled by stripAddressPrefix.
  // Other @-forms are not ownership kinds — policy denies them on ADDRESS flags.
  if (/^@worktree:/i.test(v)) return 'worktree';
  // Compound address forms used by --to
  ...
  if (/^(ctx|disp)_[A-Za-z0-9_-]+$/.test(v)) return 'dispatch';
  if (/^task_[A-Za-z0-9_-]+$/.test(v)) return 'task';
  if (/^run_[A-Za-z0-9_-]+$/.test(v)) return 'run';
  if (/^msg_[A-Za-z0-9_-]+$/.test(v)) return 'id';
  if (/^delivery_[A-Za-z0-9_-]+$/.test(v)) return 'id';
  if (/^page_[A-Za-z0-9_-]+$/.test(v)) return 'page';
  // Absolute paths / path: selectors — worktree-class
  if (v.startsWith('path:')) return 'worktree';
  if (v.startsWith('/') || /^[A-Za-z]:[\\/]/.test(v)) return 'worktree';
  if (/^(name|branch|issue|id):/.test(v)) return 'worktree';
  // Underscore orch-id tokens (auto_*, art_*, gate_*, …). Bare words
  // (permission names) and Linear keys (NAS-252) stay null.
  // Only consulted on ADDRESS flags — content flags are not value-scanned.
  if (/^[a-z][a-z0-9]*_[A-Za-z0-9_-]+$/i.test(v)) return 'id';
  return null;
}

/**
 * Normalize compound --to values (run:ID / dispatch:ID / @worktree:SEL) to the bare id.
 * @param {unknown} raw
 */
export function stripAddressPrefix(raw) {
  const v = raw == null ? '' : String(raw).trim();
  const groupWt = /^@worktree:(.+)$/i.exec(v);
  if (groupWt) return groupWt[1];
  const m = /^(run|dispatch|task|term):(.+)$/i.exec(v);
  if (m) return m[2];
  return v;
}
...
export function resolveGenericIdOwnership(id, clientKey, deps = {}) {
  ...
  // msg_* / delivery_*: NOT blanket-owned. Prior code claimed a compensating
  // --from pin; that pin was skippable. Fail closed unless explicitly listed in
  // deps.ownedMessageIds / deps.ownedDeliveryIds (tests or future mailbox index).
  if (n.startsWith('msg_')) {
    const ownedMsgs = deps.ownedMessageIds
      ? new Set([...deps.ownedMessageIds].map((x) => normalizeOpaqueId(x)).filter(Boolean))
      : null;
    if (ownedMsgs && ownedMsgs.has(n)) {
      return pack(HANDLE_OWNED, 'client_owned_message', { kind_hint: 'message' });
    }
    return pack(HANDLE_UNKNOWN, 'message_id_not_in_registry', { kind_hint: 'message' });
  }
  if (n.startsWith('delivery_')) {
    const ownedDel = deps.ownedDeliveryIds
      ? new Set([...deps.ownedDeliveryIds].map((x) => normalizeOpaqueId(x)).filter(Boolean))
      : null;
    if (ownedDel && ownedDel.has(n)) {
      return pack(HANDLE_OWNED, 'client_owned_delivery', { kind_hint: 'delivery' });
    }
    return pack(HANDLE_UNKNOWN, 'delivery_id_not_in_registry', { kind_hint: 'delivery' });
  }
...
export const INVENTORY_ALLOWLIST_KEYS = Object.freeze([
  ...
  'endedat',
  'ended_at',
  // branch/name/source/agent: recon on foreign nodes — NOT inventory by default.
  ...
  'type',
  'kind',
  // name/agent/model/source/branch stripped on non-owned (recon). Not listed.
  'role',
  'priority',
  'phase',
  'outcome',
...
const NON_OWNED_STRIP_EXTRA = new Set([
  ...
  'agent',
  'model',
  'message', // error.message recon/stack-adjacent
  'stack',
].map((k) => k.toLowerCase()));
...
export function redactOwnershipContent(
  ...
  if (Array.isArray(payload)) {
    ...
    for (const item of payload) {
      // Bare strings under a non-owned parent are content, not inventory.
      if (typeof item === 'string' && parentOwned !== true) {
        continue; // strip
      }
...
/** Diagnostic returned when stdout/stderr cannot be JSON-walked. */
export const UNWALKABLE_OUTPUT_NOTE =
  'output withheld: response was not structurally walkable (unparseable, truncated, binary, or non-JSON)';

/**
 * @deprecated Deleted fallback rewriter. Kept as a thin fail-closed stub so
 * older importers do not crash; always returns the withhold diagnostic.
 * Do not repair regex redaction — force---json + envelope walk is the path.
 */
export function redactTerminalListHumanStdout(_text, _ownedHandles) {
  return UNWALKABLE_OUTPUT_NOTE;
}

/**
 * Try to parse a CLI text field into a JSON value worth walking.
 * @param {unknown} raw
 * @returns {{ ok: true, value: unknown } | { ok: false }}
 */
function tryParseWalkableJson(raw) {
  if (typeof raw !== 'string' || !raw) return { ok: false };
  // Non-UTF8 replacement characters → treat as unwalkable binary-ish text.
  if (raw.includes('\uFFFD')) return { ok: false };
  // NULs / obvious binary
  if (raw.includes('\0')) return { ok: false };
  ...
  try {
    ...
    // Truncated / partial JSON is not walkable.
    return { ok: false };
  }
}
 ...
 *
 * Happy path: force---json envelope/JSON stdout → structural walk.
 * Fallback: if a field cannot be parsed and walked, WITHHOLD the body
 * (exit code preserved) rather than regex-rewrite human text.
 *
 * @param {object} described
 * @param {Iterable<string>|Set<string>} ownedHandles
 * @returns {object}
 */
function redactTextField(raw, ownedHandles) {
  if (typeof raw !== 'string' || !raw) return raw;
  const parsed = tryParseWalkableJson(raw);
  if (parsed.ok) {
    return JSON.stringify(redactOwnershipContent(parsed.value, ownedHandles));
  }
  return null; // signal withhold to caller
}

function withholdField(described, field) {
  const raw = described[field];
  if (typeof raw !== 'string' || !raw) return;
  described[field] = UNWALKABLE_OUTPUT_NOTE;
  if (described.output_withheld !== true) {
    described.output_withheld = true;
    described.output_withheld_reason = 'unwalkable';
  }
}
...
export function applyOwnershipListRedaction(described, ownedHandles) {
  ...
  // stderr never carries a trusted walkable inventory envelope for our purposes:
  // withhold rather than regex-rewrite (error: SECRET, banners, …).
  if (typeof described.stderr === 'string' && described.stderr) {
    const sp = tryParseWalkableJson(described.stderr);
    if (sp.ok) {
      described.stderr = JSON.stringify(redactOwnershipContent(sp.value, ownedHandles));
    } else {
      withholdField(described, 'stderr');
    }
  }
  if (typeof described.stderrTail === 'string' && described.stderrTail) {
    const sp = tryParseWalkableJson(described.stderrTail);
    if (sp.ok) {
      described.stderrTail = JSON.stringify(redactOwnershipContent(sp.value, ownedHandles));
    } else {
      withholdField(described, 'stderrTail');
    }
  }

  if (typeof described.stdout === 'string' && described.stdout) {
    const walked = redactTextField(described.stdout, ownedHandles);
    if (walked == null) {
      // Fail closed: unparseable/human/binary stdout is withheld.
      // Legitimate owned human output still works when force---json produced
      // a walkable envelope (envelope path above); only WITHHOLD when the
      // payload could carry foreign content and could not be walked.
      withholdField(described, 'stdout');
    } else {
      described.stdout = walked;
    }
  }
