...
/**
 * Write a state file, keeping it owned by whoever owned it before.
 *
 * Normal (non-root) case: a plain `writeFileSync` with the given mode — same
 * behaviour as before this module existed. When the process IS root and the
 * file already existed under another uid, the previous owner is restored, so a
 * root-run upgrade/migration cannot orphan the service account's state.
 *
 ...
 * @returns {{ ownerRestored: boolean, previousOwner: {uid:number,gid:number}|null, chownError: string|null }}
 */
export function writeFilePreservingOwner(filePath, data, opts = {}) {
  ...
  try {
    ...
    previousOwner = { uid: st.uid, gid: st.gid };
  } catch {
    previousOwner = null; // first write — nothing to preserve
  }
