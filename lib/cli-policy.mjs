...
export const FLAG_TABLE = Object.freeze((() => {
  ...
  for (const name of NON_TARGET_FLAGS) {
    t[name] = Object.freeze({ kind: FLAG_KIND.NON_TARGET, resolver: null });
  }
  for (const name of ADMIN_SELECTOR_FLAGS) {
    t[name] = Object.freeze({ kind: FLAG_KIND.ADMIN, resolver: null });
  }
  for (const [name, resolver] of Object.entries(TARGET_FLAG_RESOLVERS)) {
    t[name] = Object.freeze({ kind: FLAG_KIND.TARGET, resolver });
  }
  return t;
})());
...
export function evaluateCliArgv(args, config = {}) {
  ...
  // run-list takes no selector and returns host-wide runs — deny (no ownership
  // scope available without a --run filter the caller owns). Coordinators
  // should use their bound run via run-show --id / task-list --run.
  if (isRunList && !targetsByKind.has('run') && !targetsByKind.has('id')) {
    return denyOwnership({
      handle: null,
      status: 'unknown',
      ownedHandles: [],
      reason: 'unscoped_run_list',
      kind: 'run',
      detailOverride:
        'Blocked: orchestration run-list is unscoped and would return foreign runs. ' +
        'Pass --run <owned run id> or use run-show --id on a run this client owns.',
    });
  }
  ...
  if (targetsByKind.has('id')) {
    const values = targetsByKind.get('id');
    const denied = runChecker(
      'id',
      values,
      idOwnershipCheck,
