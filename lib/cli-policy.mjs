 ...
 * 0/false/off/no restores the warn-only migration posture and emits a
 * one-shot structured deprecation (stderr + the same cli_policy_warning
 * shape used for other policy signals). Live host flip remains an
 * operator cutover step — this module only changes the default.
 *
...
/** Explicit-off tokens that restore warn-only migration (deprecated). */
const EXPLICIT_HARDENING_OFF = new Set(['0', 'false', 'off', 'no']);

/** Structured deprecation code — same family as cli_policy_would_deny. */
export const CLI_HARDENING_OFF_DEPRECATED_CODE = 'cli_hardening_off_deprecated';

let hardeningOffDeprecationEmitted = false;

/** Test-only: reset the process-lifetime once-flag for the hatch deprecation. */
export function resetCliHardeningDeprecationForTests() {
  hardeningOffDeprecationEmitted = false;
}

/**
 * Emit the NAS-227 hatch deprecation once per process.
 *
 * Default sink matches server.mjs policy signals: one JSON line on stderr
 * with `component: 'orca-bridge'` + `event: 'cli_policy_warning'`. Tests
 * may pass `onWarning` to capture instead of writing stderr.
 *
 * @param {(warning: object) => void} [onWarning]
 */
function emitHardeningOffDeprecation(onWarning) {
  if (hardeningOffDeprecationEmitted) return;
  hardeningOffDeprecationEmitted = true;
  const warning = {
    code: CLI_HARDENING_OFF_DEPRECATED_CODE,
    message:
      'ORCA_BRIDGE_CLI_HARDENING=0|false|off|no is a deprecated escape hatch; ' +
      'the warn-only migration posture will be removed in a future minor version. ' +
      'Unset the variable to keep the default-ON exact-form allowlist.',
  };
  if (typeof onWarning === 'function') {
    try {
      onWarning(warning);
    } catch {
      // deprecation hooks must not break config resolve
    }
    return;
  }
 ...
 *   → warn-only posture + one-shot structured deprecation (stderr / onWarning)
 * Explicit on (redundant with default):
 *   ORCA_BRIDGE_CLI_HARDENING=1|true|on|yes
 * Unknown non-empty values fail closed to ON (no deprecation).
 *
 *   ORCA_BRIDGE_CLI_ADMIN=1 → unlock admin-tier forms
 *
 * @param {NodeJS.ProcessEnv | Record<string, string | undefined>} [env]
 * @param {{ onWarning?: (warning: object) => void }} [hooks]
 */
export function resolveCliPolicyConfig(env = process.env, hooks = {}) {
  const e = env || {};
  const raw = e.ORCA_BRIDGE_CLI_HARDENING;
  const hardening = envFlagDefaultOn(raw);
  if (isExplicitHardeningOff(raw)) {
    emitHardeningOffDeprecation(hooks && hooks.onWarning);
  }
  return {
    hardening,
 ...
 * @returns {boolean}
 */
function isExplicitHardeningOff(raw) {
  if (raw == null) return false;
  return EXPLICIT_HARDENING_OFF.has(String(raw).trim().toLowerCase());
}
...
function envFlagDefaultOn(raw) {
  ...
  if (v === '') return true;
  if (EXPLICIT_HARDENING_OFF.has(v)) return false;
  ...
  const rejection = {
    ...
    detail: adminRequired
      ? `Blocked by cli allowlist: "${rejectedSubcommand}" requires admin unlock ` +
        `(enable the admin toolset / ORCA_BRIDGE_CLI_ADMIN=1) while CLI hardening is on.`
      : `Blocked by exact-form cli allowlist: "${rejectedSubcommand}"` +
        (formDetail ? ` (${formDetail})` : '') +
        `. Use a documented exact-form command (action=guide raw_cli_ok / ` +
        `policy.allowed_surface). Admin-tier forms need the admin toolset.`,
    next: {
      action: 'guide',
