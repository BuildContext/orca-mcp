...
export function deriveRequiredAddressFlags(specs = CLI_COMMAND_SPECS) {
  ...
  for (const s of specs) {
    for (const p of s.positionalArgs || []) {
      ...
      if (!n || contentPos.has(n)) continue;
...
export function evaluateCliArgv(rawArgs, config = {}) {
  ...
  for (const name of flagNames) {
    ...
    if (kind === 'handle' && name === 'terminal') {
      ...
      // --worktree and handler aliases (automations --workspace via
      // getOptionalWorktreeSelector) share worktreeOwnershipCheck.
      if (name === 'worktree') {
        note('worktree', collectWorktreeSelectorsFromArgv(args));
      } else {
        // Item 5 pattern: only worktree-grammar values are ownership targets.
        // Linear --workspace UUID (classify null) is content, not a selector.
        const vals = collectFlagValuesFromArgv(args, name);
        const filtered = [];
        ...
        if (filtered.length) note('worktree', filtered);
      }
