...
/**
 * NAS-260: policy-only boolean flags allowed on one exact form.
 *
 * These names are NOT in the pinned CLI_COMMAND_SPECS snapshot (v1.4.180)
 * and MUST NOT be added there — that would break cli-argv-normalize parity
 * with shipped args.js. They are also NOT entered in FLAG_TABLE, so they
 * stay unclassified (fail-closed) on every other command.
 *
 * `--submit` on `terminal send` carries no extra authority beyond the send
 * itself (already permitted for an owned handle). Ownership preflight still
 * runs; a foreign handle is denied the same way as a send without --submit.
 *
 * @type {Readonly<Record<string, readonly string[]>>}
 */
export const POLICY_SCOPED_BOOLEAN_FLAGS = Object.freeze({
  'terminal send': Object.freeze(['submit']),
});

/**
 * @param {string|string[]|null|undefined} labelOrPath
 * @returns {readonly string[]}
 */
export function policyScopedBooleanFlagsFor(labelOrPath) {
  const label = Array.isArray(labelOrPath)
    ? labelOrPath.map((t) => String(t).toLowerCase()).join(' ')
    : String(labelOrPath || '').toLowerCase();
  return POLICY_SCOPED_BOOLEAN_FLAGS[label] || Object.freeze([]);
}
...
export function matchExactCliForm(rawArgs, opts = {}) {
  ...
  const extraAllowed = new Set(policyScopedBooleanFlagsFor(form.label));
  const notPermitted = flagNames.filter(
    (f) => !form.allowedFlags.has(f) && !extraAllowed.has(f),
  );
...
export function evaluateCliArgv(rawArgs, config = {}) {
  ...
  // NAS-260: policy-scoped booleans (today: --submit on terminal send) are
  // classified only for that exact form. They stay out of FLAG_TABLE so
  // every other command still fail-closes on the same name.
  const scopedLabel = formMatch.form?.label
    || (tokens.length >= 2 ? `${tokens[0]} ${tokens[1]}` : tokens[0] || '');
  const scopedOk = new Set(policyScopedBooleanFlagsFor(scopedLabel));
  const unclassified = collectUnclassifiedFlagsFromArgv(args).filter(
    (n) => !scopedOk.has(n),
  );
