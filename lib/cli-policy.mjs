...
export function looksLikeOwnershipGatedArgv(args) {
  ...
  // Scan for gated pairs as adjacent non-flag tokens, allowing leading flags.
  for (let i = 0; i < lower.length; i++) {
    ...
    if (t === 'orchestration') {
      const next = lower[i + 1];
      if (next == null || next === '--' || next.startsWith('-')) continue;
      if (next === 'check' || next === 'worker-read' || next === 'worker-show') {
        return true;
      }
    }
  }
...
export function evaluateCliArgv(args, config = {}) {
  ...
  // Hardening off → allow_with_warning; on → deny. Fail closed on unknown.
  //
  ...
  // Leading-flag path may be handle-gated (terminal *) or dispatch-gated
  // (orchestration worker-*) or check. Detect which kind for the funnel.
  const leadingIsDispatch =
    ...
    args.some((x, i, a) => {
      ...
      const n = String(a[i + 1] || '').toLowerCase();
      return n === 'worker-read' || n === 'worker-show';
    });
  ...
  if (ownershipCheck && (ownershipTokensGated || leadingIsHandle)) {
    ...
    const isOrchCheck =
      ...
      (leadingIsHandle &&
        ...
        args.some((x, i, a) => {
          const t = String(x).toLowerCase();
          return t === 'orchestration' && String(a[i + 1] || '').toLowerCase() === 'check';
        }));
    const toCheck =
      rawHandles.length > 0
        ? rawHandles
    ...
    if (toCheck.length) {
      ...
      if (worst) {
        const preferOwnership =
          leadingIsHandle || !hardening || prefixAllowed;
        if (preferOwnership) {
          ...
          if (leadingIsHandle && (!tokens || !tokens.length)) {
            ...
            for (let i = 0; i < lower.length; i++) {
              ...
              if (lower[i] === 'orchestration' && lower[i + 1] === 'check') {
  ...
  // Gated: orchestration worker-read / worker-show. Keyed by --dispatch, not
  // --terminal. Reuses ownershipDecision envelope (code: handle_not_owned is
  // overloaded as ownership miss; payload carries dispatch_id).
  if (dispatchOwnershipCheck && (dispatchTokensGated || leadingIsDispatch)) {
    ...
    if (worst) {
      const preferOwnership =
        leadingIsDispatch || !hardening || prefixAllowed;
      if (preferOwnership) {
        const ownedHandles = Array.isArray(worst?.owned_handles)
          ? worst.owned_handles
          : Array.isArray(worst?.ownedHandles)
        ...
        let sub = rejectedSubcommand;
        let matchedForDecision = matchedPrefix;
        if (leadingIsDispatch && (!tokens || !tokens.length)) {
          const lower = (Array.isArray(args) ? args : []).map((x) =>
            String(x).toLowerCase(),
          );
          for (let i = 0; i < lower.length; i++) {
            ...
            if (lower[i] === 'orchestration') {
              const n = lower[i + 1];
              if (n === 'worker-read' || n === 'worker-show') {
                sub = `orchestration ${n}`;
...
/**
 * Build allow_with_warning or deny for a handle/dispatch ownership miss.
 * @param {object} p
 */
