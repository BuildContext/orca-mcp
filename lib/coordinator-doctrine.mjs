...
/** Prefer this await window; client wrappers are often ~60s. Hard max matches — 240s is a wrapper trap. */
export const PREFERRED_WAIT_MS = 45_000;
export const WAIT_MS_HARD_MAX = PREFERRED_WAIT_MS;
...
export const DOCTRINE = {
  ...
  anti_patterns: [
    ...
    `single await waitMs > ${PREFERRED_WAIT_MS} when client wrapper ~60s (prefer ${PREFERRED_WAIT_MS} and re-call; hard max ${WAIT_MS_HARD_MAX})`,
...
export function buildToolDescription({ minVersion }) {
  ...
  return (
    ...
    `FORBIDDEN: worktree create --agent --prompt (rejected on cli). Prefer waitMs ${PREFERRED_WAIT_MS} (max ${WAIT_MS_HARD_MAX}).`
  );
}
