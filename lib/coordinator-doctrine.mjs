...
export function buildArgsPropertyDescription() {
  return (
    ...
    'CONTRACT: exact command path + only that form\'s flags + value grammar ' +
    '(orchestration check refuses --peek together with --all). ' +
    'OK-tier: status, worktree list|show, terminal list|read|close, skills get, ' +
    'orchestration check|reply|worker-show|worker-read|dispatch-show. ' +
    'Admin-tier (admin toolset): terminal send|create, worktree create|rm, ' +
    'orchestration send|task-create|worker-start|dispatch|run-create|run-use. ' +
    'REJECTED: worktree create --agent --prompt (use action=dispatch); anything outside the form list.'
  );
}
