...
// ---------------------------------------------------------------------------
// NAS-250 — redact by content, not by known shape (effect: no content keys)
// ---------------------------------------------------------------------------
...
describe('NAS-252 r7 redaction parity', () => {
  ...
  it('human stdout redacts without preview keyword; strips foreign titles/paths', () => {
    const described = {
      stdout:
        FOREIGN + '  orca@host: ~/src/secret  connected  /home/secret\n' +
        'subject: ' + SECRET + '\n' +
        OWN + '  mine  connected  /own\npreview: keep-me\n',
      stderr: 'payload: ' + SECRET + '\n',
      stderrTail: FOREIGN + '  title ~/x\n',
    };
    applyOwnershipListRedaction(described, [OWN]);
    assert.equal(described.stdout.includes(SECRET), false);
    assert.equal(described.stdout.includes('~/src/secret'), false);
    assert.equal(described.stdout.includes(OWN), true);
    assert.equal(described.stderr.includes(SECRET), false);
    assert.equal(described.stderrTail.includes('~/x'), false);
  });
