...
describe('normalizeTerminalHandle / getTerminalHandle', () => {
  ...
  it('returns the FIRST --terminal when the flag is duplicated', () => {
    // Mirrors cli-policy extractTerminalHandleFromArgv. Orca CLI parseArgs
    // last-wins for non-repeatable flags — first-vs-last mismatch is a known
    // NAS-247 finding (bypass when first is owned and last is foreign).
    assert.equal(
      getTerminalHandle([
        'terminal',
        'read',
...
describe('resolveTerminalHandleOwnership', () => {
  ...
  it('duplicate-flag first handle is what callers resolve (owned vs foreign)', () => {
    const src = makeSources();
    const firstOwn = getTerminalHandle([
      'terminal',
      'read',
      '--terminal',
      'term_worker_a',
    ...
    const firstForeign = getTerminalHandle([
      'terminal',
      'read',
      '--terminal',
      'term_worker_b',
    ...
    assert.equal(firstOwn, 'term_worker_a');
    assert.equal(firstForeign, 'term_worker_b');
    assert.equal(
      resolveTerminalHandleOwnership(firstOwn, 'client-a', src).status,
      HANDLE_OWNED,
    );
    assert.equal(
      resolveTerminalHandleOwnership(firstForeign, 'client-a', src).status,
      HANDLE_NOT_OWNED,
    );
  });
