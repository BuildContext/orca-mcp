...
import {
  ...
  UNWALKABLE_OUTPUT_NOTE,
  redactOwnershipContent,
  findTerminalContentKeys,
  INVENTORY_ALLOWLIST_KEYS,
  isInventoryAllowlistKey,
...
describe('redactTerminalListPayload', () => {
  ...
  it('omits preview on non-owned rows; keeps row + owned preview', () => {
    ...
    assert.equal(Object.prototype.hasOwnProperty.call(terms[1], 'title'), false); // inventory kept
  ...
  it('handles bare terminals array and top-level terminals', () => {
    ...
    assert.equal(Object.prototype.hasOwnProperty.call(outTop.terminals[0], 'title'), false);
...
describe('redactWorktreeListPayload', () => {
  it('omits preview on every worktree row', () => {
    ...
    assert.equal(Object.prototype.hasOwnProperty.call(out.result.worktrees[0], 'path'), false);
    assert.equal(
...
describe('applyOwnershipListRedaction shape-based (NAS-248 F2)', () => {
  ...
  for (const argv of ARGV_SPELLINGS) {
    it(`redacts envelope terminals regardless of argv ${JSON.stringify(argv)}`, () => {
      ...
      assert.equal(Object.prototype.hasOwnProperty.call(out[1], 'title'), false);
    });
  }
  ...
  it('withholds unwalkable human stdout instead of regex-rewriting', () => {
    const human =
      `${OWN}  mine  connected  /wt/a\n` +
      `preview: gh secret set NPM_TOKEN\n\n` +
      `${FOREIGN}  theirs  connected  /wt/b\n` +
      `preview: paste secret here\n`;
    const described = { ok: true, code: 0, stdout: human };
    applyOwnershipListRedaction(described, [OWN]);
    assert.equal(described.stdout, UNWALKABLE_OUTPUT_NOTE);
    assert.equal(described.output_withheld, true);
    assert.equal(described.code, 0);
    assert.equal(described.stdout.includes('paste secret here'), false);
    assert.equal(described.stdout.includes('NPM_TOKEN'), false);
  });

  it('redactTerminalListHumanStdout is a fail-closed stub', () => {
    const human =
      `${OWN}  mine  connected  /a\npreview: OWN_SECRET\n\n` +
      `${FOREIGN}  theirs  connected  /b\npreview: FOREIGN_SECRET\n`;
    const out = redactTerminalListHumanStdout(human, [OWN]);
    assert.equal(out, UNWALKABLE_OUTPUT_NOTE);
    assert.equal(out.includes('FOREIGN_SECRET'), false);
    assert.equal(out.includes('OWN_SECRET'), false);
  });

  it('walks JSON envelope; withholds unparseable secondary stdout', () => {
    const described = {
      ok: true,
    ...
    applyOwnershipListRedaction(described, [OWN]);
    assert.equal(described.envelope.result.status, 'alive');
    assert.equal(described.stdout, UNWALKABLE_OUTPUT_NOTE);
    assert.equal(described.output_withheld, true);
  });
});

describe('NAS-250 content redaction inversion', () => {
  const OWN = 'term_own';
  const FOREIGN = 'term_foreign';
  const SECRET = 'FOREIGN_PREVIEW_SECRET_TOKEN_xyz';

  function assertNoContentKeys(value) {
    const found = findTerminalContentKeys(value);
    assert.deepEqual(
      found,
      [],
      `expected no content-bearing keys, found ${JSON.stringify(found)} in ${JSON.stringify(value)}`,
    );
  }

  it('TERMINAL_CONTENT_KEYS covers live preview/scrollback class', () => {
    for (const k of ['preview', 'scrollback', 'buffer', 'output', 'lines', 'tail', 'snapshot']) {
      assert.ok(TERMINAL_CONTENT_KEYS.map((x) => x.toLowerCase()).includes(k));
    }
  });

  it('redacts result.terminal.preview (terminal show shape)', () => {
    const described = {
      ok: true,
      envelope: {
        ok: true,
    ...
    applyOwnershipListRedaction(described, [OWN]);
    const term = described.envelope.result.terminal;
    assert.equal(term.handle, FOREIGN);
    assert.equal(Object.prototype.hasOwnProperty.call(term, 'title'), false);
    assert.equal(term.connected, true);
    assertNoContentKeys(described.envelope);
    assert.equal(JSON.stringify(described).includes(SECRET), false);
  });

  it('keeps owned result.terminal.preview', () => {
    const described = {
      ok: true,
      envelope: {
        ok: true,
    ...
    applyOwnershipListRedaction(described, [OWN]);
    assert.equal(described.envelope.result.terminal.preview, 'OWN_OK');
  });

  const SHOW_SHAPES = [
    {
      name: 'envelope.result.terminal',
      build: () => ({
        envelope: {
          ok: true,
          result: { terminal: { handle: FOREIGN, title: 't', preview: SECRET, buffer: SECRET } },
        },
      }),
    },
    {
      name: 'stdout JSON show',
      build: () => ({
        stdout: JSON.stringify({
          ok: true,
          result: { terminal: { handle: FOREIGN, title: 't', preview: SECRET } },
        }),
      }),
    },
    {
      name: 'nested terminals[] + terminal',
      build: () => ({
        envelope: {
          result: {
            terminals: [{ handle: FOREIGN, preview: SECRET, title: 'a' }],
            terminal: { handle: FOREIGN, preview: SECRET, title: 'b' },
            extra: { nested: { handle: FOREIGN, scrollback: SECRET } },
          },
        },
      }),
    },
    {
      name: 'unknown-handle node fail-closed',
      build: () => ({
        envelope: {
          result: {
            // no handle → ownership undetermined → redact
            orphanPreview: { preview: SECRET, title: 'x' },
          },
        },
      }),
    },
  ...
  for (const shape of SHOW_SHAPES) {
    it(`deep-scan clean after redaction: ${shape.name}`, () => {
      const described = { ok: true, ...shape.build() };
      applyOwnershipListRedaction(described, [OWN]);
      if (described.envelope) assertNoContentKeys(described.envelope);
      if (described.stdout) {
        const parsed = JSON.parse(described.stdout);
        assertNoContentKeys(parsed);
      }
      assert.equal(JSON.stringify(described).includes(SECRET), false);
    });
  }

  it('redactOwnershipContent is recursive and preserves inventory', () => {
    const payload = {
      result: {
        terminals: [
          { handle: OWN, title: 'mine', preview: 'keep', state: 'connected' },
          { handle: FOREIGN, title: 'theirs', preview: SECRET, lines: [SECRET], state: 'connected' },
        ],
        counts: { total: 2 },
      },
    };
    const out = redactOwnershipContent(payload, [OWN]);
    assert.equal(out.result.terminals[0].preview, 'keep');
    assert.equal(Object.prototype.hasOwnProperty.call(out.result.terminals[1], 'title'), false); // P1-4 non-owned title stripped
    assert.equal(out.result.terminals[1].state, 'connected');
    assert.equal(out.result.counts.total, 2);
    assertNoContentKeys(out.result.terminals[1]);
  });

  it('collectWorktreeSelectorsFromArgv parses stop spellings', () => {
    assert.deepEqual(
      collectWorktreeSelectorsFromArgv(['terminal', 'stop', '--worktree', 'path:/a']),
      ['path:/a'],
    );
    assert.deepEqual(
      collectWorktreeSelectorsFromArgv(['terminal', 'stop', '--worktree=path:/b']),
      ['path:/b'],
    );
  });

  it('resolveWorktreeOwnership fail-closes unknown foreign path', () => {
    const deps = {
      dispatchRegistry: {
        list: () => [
          { dispatchId: 'd1', clientKey: 'alice', worktree: 'path:/alice/wt' },
          { dispatchId: 'd2', clientKey: 'bob', worktree: 'path:/bob/wt' },
        ],
      },
    };
    const foreign = resolveWorktreeOwnership('path:/bob/wt', 'alice', deps);
    assert.equal(foreign.status, 'not-owned');
    const own = resolveWorktreeOwnership('path:/alice/wt', 'alice', deps);
    assert.equal(own.status, 'owned');
    const unk = resolveWorktreeOwnership('path:/nobody', 'alice', deps);
    assert.equal(unk.status, 'unknown');
  });
});

describe('NAS-252 inventory allowlist redaction', () => {
  const OWN = 'term_own';
  const FOREIGN = 'term_foreign';
  const SECRET = 'SECRET_PREAMBLE_BODY_xyz';

  it('strips preamble/spec/objective/payload on non-owned nodes', () => {
    const payload = {
      result: {
        dispatch: {
          id: 'disp_1',
          task_id: 'task_1',
          assignee_handle: FOREIGN,
          status: 'running',
          preamble: SECRET,
        },
        tasks: [
          { id: 'task_1', spec: SECRET, created_by_terminal_handle: FOREIGN, status: 'open' },
        ],
        runs: [
          { id: 'run_1', objective: SECRET, coordinator_handle: FOREIGN },
        ],
        messages: [
          { id: 'm1', from_handle: FOREIGN, to_handle: OWN, subject: SECRET, body: SECRET, payload: { x: SECRET } },
        ],
      },
    };
    const out = redactOwnershipContent(payload, [OWN]);
    assert.equal(JSON.stringify(out).includes(SECRET), false);
    assert.equal(out.result.dispatch.id, 'disp_1');
    assert.equal(out.result.dispatch.assignee_handle, FOREIGN);
    assert.equal(Object.prototype.hasOwnProperty.call(out.result.dispatch, 'preamble'), false);
    assert.equal(Object.prototype.hasOwnProperty.call(out.result.tasks[0], 'spec'), false);
    assert.equal(Object.prototype.hasOwnProperty.call(out.result.runs[0], 'objective'), false);
    assert.equal(Object.prototype.hasOwnProperty.call(out.result.messages[0], 'payload'), false);
    assert.equal(Object.prototype.hasOwnProperty.call(out.result.messages[0], 'body'), false);
    assert.equal(Object.prototype.hasOwnProperty.call(out.result.messages[0], 'subject'), false);
    assert.equal(out.result.messages[0].from_handle, FOREIGN);
  });

  it('keeps content on owned assignee_handle nodes', () => {
    const payload = {
      result: {
        dispatch: {
          id: 'd',
    ...
    const out = redactOwnershipContent(payload, [OWN]);
    assert.equal(out.result.dispatch.preamble, 'OWN_PREAMBLE');
  });

  it('strips title/worktreePath on non-owned terminal list rows (P1-4)', () => {
    const payload = {
      result: {
        terminals: [
          { handle: OWN, title: 'mine path', worktreePath: '/own', preview: 'p', state: 'connected' },
          { handle: FOREIGN, title: 'orca@host: ~/src/secret', worktreePath: '/foreign', preview: 'SECRET', state: 'connected' },
        ],
      },
    };
    const out = redactOwnershipContent(payload, [OWN]);
    assert.equal(out.result.terminals[0].title, 'mine path');
    assert.equal(out.result.terminals[0].preview, 'p');
    assert.equal(out.result.terminals[1].handle, FOREIGN);
    assert.equal(out.result.terminals[1].state, 'connected');
    assert.equal(Object.prototype.hasOwnProperty.call(out.result.terminals[1], 'title'), false);
    assert.equal(Object.prototype.hasOwnProperty.call(out.result.terminals[1], 'worktreePath'), false);
    assert.equal(Object.prototype.hasOwnProperty.call(out.result.terminals[1], 'preview'), false);
  });

  it('resolveWorktreeOwnership denies synthetic on stop posture', () => {
    const deps = { ownedWorktrees: ['path:/a'] };
    const stop = resolveWorktreeOwnership('new-child', 'c', deps);
    assert.equal(stop.status, 'unknown');
    assert.equal(stop.reason, 'synthetic_create_not_claimable');
    const create = resolveWorktreeOwnership('new-child', 'c', { ...deps, allowSyntheticCreate: true });
    assert.equal(create.status, 'owned');
  });
});


describe('NAS-252 r7 redaction parity', () => {
  const OWN = 'term_own';
  const FOREIGN = 'term_foreign';
  const SECRET = 'SECRET_BODY_xyz';

  it('unwalkable human/stderr is withheld closed (no regex fallback)', () => {
    const described = {
      code: 0,
      stdout:
        FOREIGN + '  orca@host: ~/src/secret  connected  /home/secret\n' +
        'subject: ' + SECRET + '\n' +
        OWN + '  mine  connected  /own\npreview: keep-me\n',
      stderr: 'error: ' + SECRET + '\n',
      stderrTail: FOREIGN + '  title ~/x\n',
    };
    applyOwnershipListRedaction(described, [OWN]);
    assert.equal(described.stdout, UNWALKABLE_OUTPUT_NOTE);
    assert.equal(described.stderr, UNWALKABLE_OUTPUT_NOTE);
    assert.equal(described.stderrTail, UNWALKABLE_OUTPUT_NOTE);
    assert.equal(described.output_withheld, true);
    assert.equal(JSON.stringify(described).includes(SECRET), false);
    assert.equal(described.code, 0);
  });

  it('truncated/partial JSON stdout is withheld, not partially leaked', () => {
    const described = {
      code: 1,
      stdout: '{"ok":true,"result":{"preamble":"' + SECRET,
    };
    applyOwnershipListRedaction(described, [OWN]);
    assert.equal(described.stdout, UNWALKABLE_OUTPUT_NOTE);
    assert.equal(described.stdout.includes(SECRET), false);
    assert.equal(described.code, 1);
  });

  it('strips error.message, bare-string arrays, branch/name/source on non-owned', () => {
    const out = redactOwnershipContent({
      error: { message: SECRET, code: 'e', stack: SECRET },
      messages: [SECRET, 'also'],
      branch: 'feat/secret',
      name: 'recon',
      source: 'leak',
      agent: 'other',
    ...
    assert.equal(JSON.stringify(out).includes(SECRET), false);
    assert.equal(out.error?.code, 'e');
    assert.equal(Object.prototype.hasOwnProperty.call(out.error || {}, 'message'), false);
    assert.deepEqual(out.messages, []);
    assert.equal(Object.prototype.hasOwnProperty.call(out, 'branch'), false);
    assert.equal(Object.prototype.hasOwnProperty.call(out, 'name'), false);
    assert.equal(out.id, 'x');
  });

  it('classifyValueOwnershipKind maps referent grammar', () => {
    assert.equal(classifyValueOwnershipKind('term_abc'), 'handle');
    assert.equal(classifyValueOwnershipKind('run:run_1'), 'run');
    assert.equal(classifyValueOwnershipKind('dispatch:ctx_1'), 'dispatch');
    assert.equal(classifyValueOwnershipKind('delivery_1'), 'id');
    assert.equal(classifyValueOwnershipKind('msg_1'), 'id');
    assert.equal(classifyValueOwnershipKind('path:/x'), 'worktree');
    assert.equal(classifyValueOwnershipKind('@worktree:path:/x'), 'worktree');
    assert.equal(classifyValueOwnershipKind('@all'), null);
    assert.equal(classifyValueOwnershipKind('auto_x'), 'id');
    assert.equal(classifyValueOwnershipKind('NAS-252'), null);
    assert.equal(classifyValueOwnershipKind('accessibility'), null);
    assert.equal(classifyValueOwnershipKind('not-an-id'), null);
  });
});
