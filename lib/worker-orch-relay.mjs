...
import { isTemplateWorkerDone, TEMPLATE_WORKER_DONE } from './agent-tui.mjs';
...
export function parseWorkerOrchArgv(argv) {
  ...
  const parsed = {
    ...
    subject: getArgFlag(flags, '--subject') || getArgFlag(raw, '--subject'),
    body: getArgFlag(flags, '--body') || getArgFlag(raw, '--body'),
    outcome: getArgFlag(flags, '--outcome') || getArgFlag(raw, '--outcome'),
    filesModified: parseFilesModifiedFlag(flags) || parseFilesModifiedFlag(raw),
    relayArgv: ['orchestration', verb, ...flags],
  };

  if (parsed.type === 'worker_done') {
    const template = rejectTemplateWorkerDone(parsed);
    if (template) return template;
  }
  return parsed;
}

function parseFilesModifiedFlag(list) {
  const raw = getArgFlag(list, '--files-modified') || getArgFlag(list, '--payload');
  if (!raw) return null;
  try {
...
/**
 * Reject a worker_done whose subject/body/files still match the contract template.
 * @param {{ subject?: string|null, body?: string|null, filesModified?: string[]|null, outcome?: string|null }} msg
 * @returns {null | { ok: false, code: string, message: string }}
 */
export function rejectTemplateWorkerDone(msg) {
  if (isTemplateWorkerDone(msg)) {
    return {
      ok: false,
      code: TEMPLATE_WORKER_DONE,
      message: 'worker_done still matches the contract template',
    };
  }
  return null;
}
