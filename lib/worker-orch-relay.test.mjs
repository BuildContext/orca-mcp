...
describe('worker-orch-relay', () => {
  it('parses orca orchestration send worker_done', () => {
    const p = parseWorkerOrchArgv([
      ...
      '--type', 'worker_done',
      '--subject', 'done',
      '--body', 'Finished isolated harden; gitdir pointer is sticky; ACL is per-tree.',
      '--task-id', 'task_1',
      '--dispatch-id', 'ctx_abc',
      '--outcome', 'succeeded',
    ]);
  ...
  it('maps bare helper verb worker_done to send --type', () => {
    const p = parseWorkerOrchArgv([
      'worker_done',
      '--subject', 'done',
      '--body', 'Finished the isolated harden and left the tree clean.',
    ]);
    assert.equal(p.ok, true);
    assert.equal(p.verb, 'send');
    assert.equal(p.type, 'worker_done');
  ...
  it('rejects template worker_done (subject/body/filesModified)', () => {
    const p = parseWorkerOrchArgv([
      'orchestration', 'send',
      '--type', 'worker_done',
    ...
    assert.equal(p.ok, false);
    assert.equal(p.code, 'template_worker_done');
  });

  it('rejects empty-body worker_done even with outcome succeeded', () => {
    const p = parseWorkerOrchArgv([
      'send', '--type', 'worker_done',
      '--subject', 'done',
      '--body', '',
      '--outcome', 'succeeded',
    ]);
