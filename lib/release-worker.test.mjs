...
describe('executeReleaseWorker effect ordering (NAS-248 F1 / NAS-202)', () => {
  ...
  it('owned handle close-fallback after worker-release miss still works', async () => {
    ...
    assert.ok(
      calls.some((c) => c.includes('terminal close') && !c.includes('--tab')),
      'bridge release close must not pass --tab',
    );
  });

  it('NAS-261: tab_not_found on close is already_gone success; purge still runs', async () => {
    const purges = [];
    const closeArgv = [];
    const result = await executeReleaseWorker(
      { dispatch_id: OWN_D, terminal_handle: OWN_H },
      {
        ...
        runJson: async (argv) => {
          if (argv[1] === 'worker-release') {
            return { ok: false, envelope: { ok: false, error: { code: 'dispatch_not_found' } } };
          }
          if (argv[0] === 'terminal' && argv[1] === 'close') {
            closeArgv.push(argv.slice());
            return {
              ok: false,
              envelope: { ok: false, error: { code: 'tab_not_found', message: 'tab_not_found' } },
            };
          }
          return { ok: false, envelope: { ok: false } };
        },
    ...
    assert.equal(result.close_code, 'tab_not_found');
    assert.equal(purges.length, 1);
    assert.ok(!closeArgv[0].includes('--tab'));
  });

  it('NAS-261: workspace_session_unavailable is already gone', async () => {
    const result = await executeReleaseWorker(
      { dispatch_id: OWN_D, terminal_handle: OWN_H },
      {
        clientKey: 'alice',
        ownershipDeps: ownershipDeps(),
        runJson: async (argv) =>
          argv[1] === 'worker-release'
            ? { ok: false, envelope: { ok: false, error: { code: 'dispatch_not_found' } } }
            : {
                ok: false,
                envelope: { ok: false, error: { code: 'workspace_session_unavailable' } },
              },
        envOk,
        pick,
        releaseRefusesCoordinator,
        coordinatorHandles: new Set([SENDER]),
        upsertDispatch: () => {},
        purgeWorkerCreds: async () => ({ ok: true }),
      },
    );
    assert.equal(result.ok, true);
    assert.equal(result.already_gone, true);
    assert.equal(result.close_code, 'workspace_session_unavailable');
  });

  it('purges only after close when no other isolated dispatch is running', async () => {
    const calls = [];
    const purges = [];
    const result = await executeReleaseWorker(
      { dispatch_id: OWN_D, terminal_handle: OWN_H },
      {
        ...
        purgeWorkerCreds: async (opts = {}) => {
          purges.push(opts);
          return { ok: true, cap: { ok: true, dispatchId: opts.dispatchId } };
        },
      },
    );
    assert.equal(result.ok, true);
    assert.equal(result.credential_purge.cap.ok, true);
    assert.equal(result.credential_purge.cap.dispatchId, OWN_D);
    assert.equal(purges.length, 1);
    assert.ok(calls.some((call) => call.includes('terminal close')));
  });

  it('keeps shared credential while another isolated dispatch is running', async () => {
    const deps = ownershipDeps();
    deps.dispatchRegistry.list = () => [
      { dispatchId: OWN_D, clientKey: 'alice', terminalHandle: OWN_H },
      { dispatchId: 'disp_other', status: 'running', workerIsolation: true },
    ];
    let lastPurge = null;
    const result = await executeReleaseWorker(
      { dispatch_id: OWN_D, terminal_handle: OWN_H },
      {
        ...
        runJson: async (argv) =>
          argv[1] === 'worker-release'
            ? { ok: false, envelope: { ok: false, error: { code: 'dispatch_not_found' } } }
            : { ok: true, envelope: { ok: true, result: { closed: true } } },
        envOk,
        pick,
        releaseRefusesCoordinator,
        coordinatorHandles: new Set([SENDER]),
        ...
        purgeWorkerCreds: async (opts = {}) => {
          lastPurge = opts;
          return { ok: true, cap: { ok: true }, creds: { skipped: 'other_live_isolated_dispatch' } };
        },
      },
    );
    assert.equal(result.credential_purge.skipped, 'other_live_isolated_dispatch');
    assert.equal(lastPurge?.creds, false);
    assert.equal(lastPurge?.dispatchId, OWN_D);
  });

  it('successful worker-release does not purge (live close-path-only behaviour)', async () => {
    const purges = [];
    const result = await executeReleaseWorker(
      { dispatch_id: OWN_D, terminal_handle: OWN_H },
      {
    ...
    assert.equal(result.ok, true);
    assert.equal(result.mode, 'worker-release');
    assert.equal(result.credential_purge, undefined);
    assert.deepEqual(purges, []);
  });

  function closePathCtx(overrides = {}) {
    return {
      clientKey: 'alice',
      ownershipDeps: ownershipDeps(),
      runJson: async (argv) =>
  ...
  it('NAS-271: skips credential purge when isolation is off and nothing was minted', async () => {
    const deps = ownershipDeps();
    deps.dispatchRegistry.list = () => [
      {
        dispatchId: OWN_D,
    ...
    const result = await executeReleaseWorker(
      ...
      closePathCtx({
        ...
        purgeWorkerCreds: async (opts = {}) => {
          purges.push(opts);
          return { ok: false, error: 'privileged purge must not run' };
        },
      }),
    );
    assert.equal(result.ok, true);
    assert.deepEqual(purges, []);
    assert.equal(result.credential_purge?.skipped, 'not_provisioned');
    assert.notEqual(result.error, 'credential_purge_failed');
  });

  it('NAS-271: failed purge still fails release when a capability was minted', async () => {
    const deps = ownershipDeps();
    deps.dispatchRegistry.list = () => [
      {
        dispatchId: OWN_D,
    ...
    const result = await executeReleaseWorker(
      ...
      closePathCtx({
        ...
        purgeWorkerCreds: async (opts = {}) => {
          ...
          return {
            ok: false,
            cap: { ok: false, error: 'sudo: a password is required' },
            creds: { ok: false, error: 'sudo: a password is required' },
          };
        },
      }),
    );
    assert.equal(purges.length, 1);
    assert.equal(result.ok, false);
    assert.equal(result.error, 'credential_purge_failed');
    assert.equal(result.credential_purge.ok, false);
  });
