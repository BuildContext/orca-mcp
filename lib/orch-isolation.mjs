 ...
 *
 * Fail-closed (NAS-248 P0 #4 / N2): an empty owned set means "no authority",
 * not "everything is mine". Bridge restart must restore durable bindings rather
 * than re-claim mailbox rows on read. Messages with no dispatch id cannot be
 * released by id; they stay in `own` only when the caller already has at least
 * one registered dispatch (so a live owner can still see unscoped chatter).
 *
...
export function partitionMailbox(messages, ownedDispatchIds, pick) {
  ...
  // Empty owned set: nothing is authoritative. Treat every message as foreign;
  // never fail-open into a claim path.
  if (owned.size === 0) {
    if (list.length === 0) {
      return { own: [], foreign: [], filtered: false };
    }
    return { own: [], foreign: list, filtered: true };
  }
