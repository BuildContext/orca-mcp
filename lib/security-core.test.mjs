...
import {
  ...
  extractBearer,
  injectSenderArgv,
  applySpawnPathSenderInject,
  orchestrationNeedsSenderPin,
...
describe('argv builders: dispatch path', () => {
  ...
  it('buildTerminalSendArgv submits with --enter (there is no --submit)', () => {
    assert.deepEqual(
      buildTerminalSendArgv({ handle: 'term_w', text: 'hi' }),
      ['terminal', 'send', '--terminal', 'term_w', '--text', 'hi', '--enter', '--json'],
    );
    assert.deepEqual(
      buildTerminalSendArgv({ handle: 'term_w' }),
      ['terminal', 'send', '--terminal', 'term_w', '--enter', '--json'],
    );
    assert.deepEqual(
      buildTerminalSendArgv({ handle: 'term_w', text: '', enter: true }),
      ['terminal', 'send', '--terminal', 'term_w', '--text', '', '--enter', '--json'],
    );
    const argv = buildTerminalSendArgv({ handle: 'term_w', text: 'x', enter: true });
    assert.ok(argv.includes('--enter'));
    assert.ok(!argv.includes('--submit'));
  });

  it('runtime dispatch --inject cannot express Enter; submit is a separate terminal send', () => {
    const inject = buildDispatchInjectArgv({ taskId: 'task_1', handle: 'term_w' });
    assert.ok(inject.includes('--inject'));
    assert.ok(!inject.includes('--enter'));
    assert.ok(!inject.includes('--submit'));
    const submit = buildTerminalSendArgv({ handle: 'term_w' });
    assert.ok(submit.includes('--enter'));
    assert.ok(!submit.includes('--text'));
  });

  // Live 2026-08-15: a Grok TUI compose box did not submit on
  // `--text <payload> --enter` alone (brief sat in the draft buffer).
  // A following empty-text `--enter` submitted it. A plain bash shell
  // submits on the first Enter; the second send is then a no-op.
  // Do not "simplify" the two-send sequence away again.
  it('TUI text sends emit text+enter then empty+enter (live 2026-08-15 Grok draft buffer)', () => {
    const src = fs.readFileSync(new URL('../server.mjs', import.meta.url), 'utf8');
    assert.match(src, /buildTerminalSendArgv/);
    assert.match(src, /step: 'dispatch-inject-enter'/);
    assert.equal(/['"]--submit['"]/.test(src), false);
    const twoSend = /buildTerminalSendArgv\(\{\s*handle,\s*text: \w+,\s*enter: true\s*\}\)[\s\S]{0,500}?buildTerminalSendArgv\(\{\s*handle,\s*text: '',\s*enter: true\s*\}\)/g;
    const matches = src.match(twoSend) || [];
    assert.equal(
      matches.length,
      2,
      'idle-recovery and dispatch-preamble must both emit text --enter then empty --enter',
    );
    assert.match(src, /enterOk:\s*envOk\(enterRes\)/);
    assert.match(src, /Grok TUI draft buffer/);
    assert.match(src, /verified live[\s\S]{0,20}2026-08-15/);
  });
});
...
describe('argv builders: await / check', () => {
  ...
  it('injectSenderArgv adds --terminal for check and --from for mutations', () => {
    ...
    // already present — bridge OVERWRITES with pin (never honour caller value)
    ...
    assert.deepEqual(already.slice(-2), ['--terminal', 'term_sender']);

    // --from caller value always overwritten
    const spoofed = injectSenderArgv(
      ['orchestration', 'reply', '--id', 'msg_x', '--from', 'term_FOREIGN', '--body', 'x'],
      'term_sender',
    );
    assert.ok(spoofed.includes('term_sender'));
    assert.ok(!spoofed.includes('term_FOREIGN'));
    assert.equal(spoofed[spoofed.indexOf('--from') + 1], 'term_sender');
    ...
    // NAS-252 P0-3: inbox is pin-scoped like check
    const inbox = injectSenderArgv(
      ['orchestration', 'inbox', '--json', '--limit', '5'],
      'term_sender',
    );
    assert.deepEqual(inbox.slice(-2), ['--terminal', 'term_sender']);
  });
  ...
  it('spawn path withSender gates inbox (not just injectSenderArgv helper)', () => {
    // P0 r7: server.mjs withSender must match this helper.
    assert.deepEqual(orchestrationNeedsSenderPin('check'), { needsFrom: false, needsTerminal: true });
    assert.deepEqual(orchestrationNeedsSenderPin('inbox'), { needsFrom: false, needsTerminal: true });
    assert.equal(orchestrationNeedsSenderPin('reply').needsFrom, true);
    assert.equal(orchestrationNeedsSenderPin('worker-list').needsTerminal, false);

    const inbox = applySpawnPathSenderInject(
      ['orchestration', 'inbox', '--json', '--limit', '3'],
      'term_pin',
    );
    assert.deepEqual(inbox.slice(-2), ['--terminal', 'term_pin']);

    const untouched = applySpawnPathSenderInject(
      ['orchestration', 'worker-list', '--json'],
      'term_pin',
    );
    assert.deepEqual(untouched, ['orchestration', 'worker-list', '--json']);
  });
});

describe('NAS-239: escalation reply dual-route helpers', () => {
  it('DEFAULT_WAIT_TYPES stays a comma string without status/heartbeat', () => {
    assert.equal(typeof DEFAULT_WAIT_TYPES, 'string');
    assert.equal(DEFAULT_WAIT_TYPES, 'worker_done,escalation,question');
    assert.ok(!DEFAULT_WAIT_TYPES.split(',').includes('status'));
    assert.ok(!DEFAULT_WAIT_TYPES.split(',').includes('heartbeat'));
  });

  it('parseOrchestrationReplyArgv extracts flags and rejects non-reply', () => {
    assert.equal(parseOrchestrationReplyArgv(['orchestration', 'check']), null);
    assert.equal(parseOrchestrationReplyArgv(['worktree', 'list']), null);
    const missing = parseOrchestrationReplyArgv([
      'orchestration',
      'reply',
      '--body',
      'hi',
    ...
    const parsed = parseOrchestrationReplyArgv([
      'orchestration',
      'reply',
      '--id',
      'msg_esc1',
    ...
    assert.equal(parsed.id, 'msg_esc1');
    assert.equal(parsed.body, 'try ask instead');
    assert.equal(parsed.run, 'run_1');
    assert.equal(parsed.from, 'term_coord');
    assert.equal(parsed.json, true);

    const eq = parseOrchestrationReplyArgv([
      'orchestration',
      'reply',
      '--id=msg_x',
      '--body=hello world',
    ...
    assert.equal(eq.id, 'msg_x');
    assert.equal(eq.body, 'hello world');
  });

  it('extractDispatchIdFromMessage reads payload and dispatch: handles', () => {
    assert.equal(extractDispatchIdFromMessage(null), null);
    assert.equal(
      extractDispatchIdFromMessage({
        type: 'escalation',
        from_handle: 'dispatch:ctx_abc',
        payload: JSON.stringify({ dispatchId: 'ctx_abc', taskId: 'task_1' }),
      }),
      'ctx_abc',
    );
    assert.equal(
      extractDispatchIdFromMessage({
        payload: { dispatch_id: 'ctx_nested' },
      }),
      'ctx_nested',
    );
    assert.equal(
      extractDispatchIdFromMessage({ dispatchId: 'dispatch:ctx_top' }),
      'ctx_top',
    );
  });

  it('resolveWorkerDispatchId prefers original then workers map', () => {
    assert.equal(
      resolveWorkerDispatchId({
        originalMessage: {
          type: 'escalation',
          payload: '{"dispatchId":"ctx_from_esc"}',
        },
        replyMessage: { to_handle: 'term_worker' },
      }),
      'ctx_from_esc',
    );
    ...
    assert.equal(
      resolveWorkerDispatchId({
        originalMessage: { from_handle: 'term_w1' },
        replyMessage: { to_handle: 'term_w1' },
        workers: [
          {
            dispatchId: 'ctx_mapped',
            agentTerminalHandle: 'term_w1',
          },
        ],
      }),
      'ctx_mapped',
    ...
    assert.equal(
      resolveWorkerDispatchId({
        replyMessage: { to_handle: 'term_unknown' },
        workers: [],
      }),
      null,
    );
  ...
  it('buildEscalationReplyFollowupSendArgv targets dispatch mailbox as status', () => {
    const argv = buildEscalationReplyFollowupSendArgv({
      dispatchId: 'ctx_worker',
      body: 'continue with plan B',
      subject: 'Re: Blocked',
      threadId: 'msg_esc1',
      runId: 'run_9',
    });
    ...
    // already-prefixed id does not double
    const argv2 = buildEscalationReplyFollowupSendArgv({
      dispatchId: 'dispatch:ctx_x',
      body: 'x',
    });
    assert.equal(argv2[argv2.indexOf('--to') + 1], 'dispatch:ctx_x');
    assert.throws(() => buildEscalationReplyFollowupSendArgv({ body: 'x' }), /dispatchId/);
  });

  it('replyEnvelopeIsQuestionAnswer detects question_threads path', () => {
    assert.equal(replyEnvelopeIsQuestionAnswer(null), false);
    assert.equal(
      replyEnvelopeIsQuestionAnswer({
        ok: true,
        result: { message: { type: 'status' }, question: { message_id: 'msg_q' } },
      }),
      true,
    );
    assert.equal(
      replyEnvelopeIsQuestionAnswer({
        ok: true,
        result: { message: { type: 'status', to_handle: 'term_w' } },
      }),
      false,
    );
  });

  it('explainStaleDeliveryError distinguishes msg_ ack from delivery_ ack', () => {
    assert.equal(isStaleDeliveryError('stale_delivery', 'x'), true);
    assert.equal(
      isStaleDeliveryError('other', 'Delivery msg_x does not belong to this Run.'),
      true,
    );
    assert.equal(isStaleDeliveryError('consumer_fenced', 'fenced'), false);

    const msgAck = explainStaleDeliveryError({
      ackId: `${MESSAGE_ID_PREFIX}4a6262180e1c`,
      errorCode: 'stale_delivery',
      errorMessage: 'Delivery msg_4a6262180e1c does not belong to this Run.',
    });
    assert.equal(msgAck.hint, 'ack_message_id_not_delivery_id');
    assert.match(msgAck.message, /message id, not a delivery id/);
    assert.match(msgAck.message, /deliveryId/);

    const delAck = explainStaleDeliveryError({
      ackId: `${DELIVERY_ID_PREFIX}abc`,
      errorCode: 'stale_delivery',
      errorMessage: 'Delivery delivery_abc does not belong to this Run.',
    });
    assert.equal(delAck.hint, 'delivery_not_in_run_or_wrong_generation');
    assert.match(delAck.message, /consumer generation/);
