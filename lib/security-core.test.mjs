...
describe('argv builders: dispatch path', () => {
  ...
  it('buildDispatchCurrentTerminalArgv prefixes bare absolute paths with path:', () => {
    assert.deepEqual(
      buildDispatchCurrentTerminalArgv({ repo: '/home/USER/r', name: 't', agent: 'omp' }),
      ['terminal', 'create', '--worktree', 'path:/home/USER/r', '--title', 't', '--command', 'omp', '--json'],
    );
    assert.deepEqual(
      buildDispatchCurrentTerminalArgv({ repo: 'path:/home/USER/r', name: 't', agent: 'omp' }),
      ['terminal', 'create', '--worktree', 'path:/home/USER/r', '--title', 't', '--command', 'omp', '--json'],
    );
  });
