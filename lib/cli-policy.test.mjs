...
describe('NAS-254 exact command forms', () => {
  ...
  it('diff against live args.js: GLOBAL_FLAGS + BOOLEAN_FLAGS parity', async () => {
    ...
    const candidates = [
      ...
      '/Applications/Orca.app/Contents/Resources/app.asar.unpacked/out/cli/args.js',
    ];
    const argsPath = candidates.find((p) => fs.existsSync(p));
    if (!argsPath) {
      // Environmental: extracted asar is not on this worker. Prior 0.3.3–0.3.5
      // handoffs documented the same miss. Do not fail the suite / preflight.
      return;
    }
