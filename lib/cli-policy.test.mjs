...
describe('evaluateCliArgv: handle ownership', () => {
  ...
  it('missing ownershipCheck fail-closes when --terminal is present', () => {
    ...
    assert.equal(r.decision, 'deny');
    assert.equal(r.rejection?.code, 'handle_not_owned');
    assert.equal(r.rejection?.reason, 'ownership_check_not_configured');
  });
...
describe('evaluateCliArgv: ownership argv-shape edges', () => {
  ...
  it('`--` does not hide --terminal from ownership default-deny', () => {
    ...
    assert.equal(rBefore.rejection?.handle, FOREIGN);
    ...
    assert.equal(rAmong.rejection?.handle, FOREIGN);
  });
...
describe('NAS-248 P1: interleaved globals still ownership-gate (hardening off)', () => {
  ...
  for (const argv of interleaved) {
    it(`denies with hardening OFF: ${argv.join(' ')}`, () => {
      ...
      if (isDispatch) {
        assert.equal(r.rejection?.ownership_kind, 'dispatch');
        assert.equal(r.rejection?.dispatch_id, 'disp_foreign');
      } else {
        assert.equal(r.rejection?.handle, FOREIGN);
      }
    });
...
describe('NAS-250/251/252: selector default-deny effects', () => {
  ...
  function assertDenied(r, { kind, target } = {}) {
    ...
    assert.equal(r.rejection?.error, 'cli_policy_denied');
    if (kind) assert.equal(r.rejection?.ownership_kind, kind);
    if (kind === 'handle' && target !== undefined) {
      assert.equal(r.rejection?.handle, target);
    }
    if (kind === 'dispatch' && target !== undefined) {
      assert.equal(r.rejection?.dispatch_id, target);
    }
    if (kind === 'worktree' && target !== undefined) {
      assert.equal(r.rejection?.worktree, target);
    }
  }
