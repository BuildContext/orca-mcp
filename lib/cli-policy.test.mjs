...
describe('NAS-254 exact command forms', () => {
  ...
  it('diff against live args.js: GLOBAL_FLAGS + BOOLEAN_FLAGS parity', async () => {
    const fs = await import('node:fs');
    const path = await import('node:path');
    const { GLOBAL_FLAGS, BOOLEAN_FLAGS } = await import('./cli-argv-normalize.mjs');
    const candidates = [
      '/tmp/nas-248-cli/squashfs-root/resources/app.asar.unpacked/out/cli/args.js',
      '/tmp/orca-asar/extracted/out/cli/args.js',
    ];
    const argsPath = candidates.find((p) => fs.existsSync(p));
    assert.ok(argsPath, 'live args.js must be present for differential');
    ...
    void path; // keep import used under lint-ish envs
  });
...
describe('NAS-260 terminal send --submit (owned-handle boolean only)', () => {
  const adminOwned = {
    hardening: true,
    admin: true,
    ...allOwnedCheckers({
  ...
  it('is scoped to terminal send and is not a global FLAG_TABLE grant', () => {
    assert.deepEqual([...POLICY_SCOPED_BOOLEAN_FLAGS['terminal send']], ['submit']);
    assert.deepEqual([...policyScopedBooleanFlagsFor(['terminal', 'send'])], ['submit']);
    assert.deepEqual([...policyScopedBooleanFlagsFor('orchestration send')], []);
    assert.equal(FLAG_TABLE.submit, undefined);
    assert.ok(
      collectUnclassifiedFlagsFromArgv(['status', '--submit']).includes('submit'),
    );
  ...
  it('owned handle + --submit allows (flag is not stripped)', () => {
    const argv = [
      'terminal', 'send',
      '--terminal', 'term_own',
      '--text', 'hi',
  ...
  it('foreign handle + --submit denies ownership (preflight not weakened)', () => {
    const r = evaluateCliArgv(
      ['terminal', 'send', '--terminal', 'term_FOREIGN', '--text', 'hi', '--submit'],
      adminOwned,
    );
    assert.equal(r.ok, false);
    assert.equal(r.decision, 'deny');
  ...
  it('unknown flag on terminal send still fail-closes (no global grant)', () => {
    const r = evaluateCliArgv(
      ['terminal', 'send', '--terminal', 'term_own', '--text', 'hi', '--not-a-real-flag'],
      adminOwned,
    );
    assert.equal(r.ok, false);
    assert.equal(r.decision, 'deny');
    ...
    assert.match(String(r.rejection?.detail), /--not-a-real-flag/);
  });

  it('--submit on any other command stays unclassified / denied', () => {
    for (const argv of [
      ['status', '--submit'],
      ['orchestration', 'send', '--to', 'term_own', '--subject', 's', '--submit'],
    ]) {
      const r = evaluateCliArgv(argv, adminOwned);
